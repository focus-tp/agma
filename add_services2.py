...
# Since regex is fragile, I'll use replace with a known unique string
volosy_end = '</div>\n              </div>\n            </div>\n\n            <!-- Ногти -->'
html = html.replace(volosy_end, additional_hair + '\n                ' + volosy_end)

nails_match = re.search(r'(<h3>Ногти</h3>\s*<span class="accordion-icon"></span>\s*</button>\s*<div class="accordion-content">).*?(</div>\n\s*</div>\n\s*<!-- Макияж -->)', html, re.DOTALL)
if nails_match:
    html = html.replace(nails_match.group(0), nails_match.group(1) + '\n' + new_nails + '\n              ' + nails_match.group(2))
